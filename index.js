...
//		this.initVariables();
	...
	login() {
		...
		this.socket = new WebSocket('ws://'+this.config.host+':'+this.config.port+'/api/v4/');
		...
		return [
			{
				...
				width: 12,
				label: 'Information',
				value: 'This will connect with Rational Acoustics Smaart server.<br> If using Smaart older than V9 this module will not work!'
			},
			{
				type: 'textinput',
				id: 'host',
...
/*			'selectTabByName': {
			...
			'moveFrontTrace': {
				label: 'Top Trace Trace Y Offset',
				options: [
					{
						type: 'dropdown',
						label: 'Direction',
